...
class AmazonInstanceEpt(models.Model):
    ...
    def get_fba_total_orders(record, fulfillment_by_where_clause):
        ...
        def orders_of_current_year(record, fulfillment_by_where_clause):
            ...
            amz_fulfillment_by = 'FBA' {fulfillment_by_where_clause} order by date(date_order
            )""".format(fulfillment_by_where_clause=fulfillment_by_where_clause)
            record._cr.execute(query, (record.id,))
            return record._cr.dictfetchall()
    ...
    def get_compare_data(record, fulfillment_by_where_clause):
        ...
        def get_compared_week_data(record, fulfillment_by_where_clause):
            ...
            and (select date_trunc('week', (select date_trunc('week', current_date) - interval '7 day')) + interval 
            '{day_of_week} day') and amz_instance_id=%s and state in ('sale','done') {fulfillment_by_where_clause}""".format(
        ...
        def get_compared_month_data(record, fulfillment_by_where_clause):
            ...
            select date_trunc('month', current_date) - interval '1 month')) + interval '{day_of_month} days') and amz_instance_id=%s and state in (
            'sale','done') {fulfillment_by_where_clause}""".format(day_of_month=day_of_month, fulfillment_by_where_clause=fulfillment_by_where_clause)
        ...
        def get_compared_year_data(record, fulfillment_by_where_clause):
            ...
            previous_year_query = """select sum(amount_untaxed) as previous_year from sale_order where date(date_order)
                            ...
                            """ .format(delta=delta, fulfillment_by_where_clause=fulfillment_by_where_clause)
    ...
    def get_graph_data(record, fulfillment_by_where_clause):
        ...
        def graph_of_all_time(record, fulfillment_by_where_clause):
            query = """select TRIM(TO_CHAR(DATE_TRUNC('month',date_order),'YYYY-MM')),sum(amount_untaxed)
                                ...
                                order by DATE_TRUNC('month',date_order)""".format(fulfillment_by_where_clause=fulfillment_by_where_clause)
            record._cr.execute(query, (record.id,))
            return record._cr.dictfetchall()
    ...
    def get_fbm_total_orders(record, fulfillment_by_where_clause):
        ...
        def orders_of_current_week(record, fulfillment_by_where_clause):
            query = """select id from sale_order where date(date_order) >= (select date_trunc('week', date(current_date))) 
            and amz_instance_id= %s and state in ('sale','done') and amz_fulfillment_by = 'FBM' {fulfillment_by_where_clause} order by date(
            date_order)""".format(fulfillment_by_where_clause=fulfillment_by_where_clause)
            record._cr.execute(query, (record.id,))
            return record._cr.dictfetchall()
        ...
        def orders_of_all_time(record, fulfillment_by_where_clause):
            query = """select id from sale_order where amz_instance_id = %s and state in ('sale','done') and 
            amz_fulfillment_by = 'FBM' {fulfillment_by_where_clause}""".format(fulfillment_by_where_clause=fulfillment_by_where_clause)
            record._cr.execute(query, (record.id,))
            return record._cr.dictfetchall()
